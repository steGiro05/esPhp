...
<body>
    <h1>Film</h1>
    <form action="sceltaFilm.php" method="post">
        <label for="scelta">Seleziona una opzione:</label>
        <select id="scelta" name="scelta">
            <option value="" <?php echo (empty($_SESSION['scelta'])) ? 'selected' : ''; ?>></option>
            <?php
            foreach ($film as $value) {
                $selected = ($_SESSION['scelta'] == $value['id']) ? 'selected' : '';
